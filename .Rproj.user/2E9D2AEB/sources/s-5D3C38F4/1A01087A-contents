#' @title Catches by species, fishing mode, year, fleet and ocean
#' @description Catches by species, fishing mode, year, fleet and ocean.
#' @name avdth_catches_sp_fishingmode_year_fleet_ocean
...
#' tmp1 <- avdth_catches_sp_fishingmode_year_fleet_ocean(avdth_con = avdth_connection,
#'                                                       year = 2017,
#'                                                       fleet = c(1, 41),
#'                                                       ocean = 1,
#'                                                       fishing_mode = 1
...
#' tmp2 <- avdth_catches_sp_fishingmode_year_fleet_ocean(avdth_con = avdth_connection,
#'                                                       year = 2017,
#'                                                       fleet = c(1, 41),
#'                                                       ocean = 1,
#'                                                       fishing_mode = 1
...
#'                                                       specie = c("ALB", "BLT"))
#' @export
avdth_catches_sp_fishingmode_year_fleet_ocean <- function (avdth_con,
                                                           year,
                                                           fleet,
                                                           ocean,
                                                           fishing_mode,
  ...
  # Query importation ----
  avdth_catches_sp_fishingmode_year_fleet_ocean_query <- paste(readLines(con = system.file("sql",
                                                                                           "avdth_catches_sp_fishingmode_year_fleet_ocean.sql",
                                                                                           package = "fishi")),
                                                               collapse = "\n")
  ...
  avdth_catches_sp_fishingmode_year_fleet_ocean <- DBI::dbGetQuery(avdth_con,
                                                                   avdth_catches_sp_fishingmode_year_fleet_ocean_query)
  # Data design ----
  # Ocean name
  if (unique(avdth_catches_sp_fishingmode_year_fleet_ocean$ocean) == 1) {
    ocean_name <- "Atlantic Ocean"
  } else {
    if (unique(avdth_catches_sp_fishingmode_year_fleet_ocean$ocean) == 2) {
      ocean_name <- "Indian Ocean"
    } else {
      if (unique(avdth_catches_sp_fishingmode_year_fleet_ocean$ocean) == 3) {
        ocean_name <- "West Pacific Ocean"
      } else {
        if (unique(avdth_catches_sp_fishingmode_year_fleet_ocean$ocean) == 4) {
          ocean_name <- "East Pacific Ocean"
        } else {
          if (unique(avdth_catches_sp_fishingmode_year_fleet_ocean$ocean) == 5) {
            ocean_name <- "Pacific Ocean"
          } else {
            if (unique(avdth_catches_sp_fishingmode_year_fleet_ocean$ocean) == 6) {
  ...
  # Fishing mode
  fishing_mode <- ifelse(unique(avdth_catches_sp_fishingmode_year_fleet_ocean$fishing_mode) == "BL",
                         "free school",
                         ifelse(unique(avdth_catches_sp_fishingmode_year_fleet_ocean$fishing_mode) == "BO",
                                "floating object",
                                "undetermined school"))
  # Species selection
  if (unique(specie != "all")) {
    avdth_catches_sp_fishingmode_year_fleet_ocean <- dplyr::filter(.data = avdth_catches_sp_fishingmode_year_fleet_ocean,
