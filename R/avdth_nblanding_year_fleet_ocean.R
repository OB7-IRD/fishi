...
#' @name avdth_nblanding_year_fleet_ocean
...
#' @export
avdth_nblanding_year_fleet_ocean <- function (avdth_con,
                                              year,
                                              fleet,
                                              ocean,
                                              fleet_name) {
  ...
  if (unique(avdth_nblanding_fleet_ocean$ocean) == 1) {
    ocean_name <- "Atlantic Ocean"
  } else {
    if (unique(avdth_nblanding_fleet_ocean$ocean) == 2) {
      ocean_name <- "Indian Ocean"
    } else {
      if (unique(avdth_nblanding_fleet_ocean$ocean) == 3) {
        ocean_name <- "West Pacific Ocean"
      } else {
        if (unique(avdth_nblanding_fleet_ocean$ocean) == 4) {
          ocean_name <- "East Pacific Ocean"
        } else {
          if (unique(avdth_nblanding_fleet_ocean$ocean) == 5) {
            ocean_name <- "Pacific Ocean"
          } else {
            if (unique(avdth_nblanding_fleet_ocean$ocean) == 6) {
  ...
  avdth_nblanding_fleet_ocean_final <- as.data.frame(x = matrix(data = c(rep(year, 12),
                                                                         1:12,
                                                                         rep(ocean, 12)),
                                                                nrow = 12,
                                                                ncol = 3,
