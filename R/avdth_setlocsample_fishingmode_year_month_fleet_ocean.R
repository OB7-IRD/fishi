#' @name avdth_setlocsample_fishingmode_year_month_fleet_ocean
#' @title Sampled sets location by year, month, fleet, fishing mode and ocean (associated to an AVDTH database)
#' @description Sampled sets location by year, month, fleet, fishing mode and ocean (associated to an AVDTH database).
...
#' \dontrun{
#' tmp <- avdth_setlocsample_fishingmode_year_month_fleet_ocean(avdth_con = avdth_connection,
#'                                                              year = 2017,
#'                                                              fleet = c(1 , 41),
#'                                                              ocean = 1,
#'                                                              fishing_mode = 1,
...
avdth_setlocsample_fishingmode_year_month_fleet_ocean <- function(avdth_con,
                                                                  year,
                                                                  fleet,
                                                                  ocean,
                                                                  fishing_mode,
  ...
  if (min(avdth_setlocsample_fishingmode_year_month_fleet_ocean_final$longitude_dec) < lon_min_max[1]
      ...
      | max(avdth_setlocsample_fishingmode_year_month_fleet_ocean_final$latitude_dec) > lat_min_max[2]) {
    warning("Be careful!",
            "\n",
  ...
  names(tmp) <- paste(month.abb[min(unique(avdth_setlocsample_fishingmode_year_month_fleet_ocean_final$month_set)):max(unique(avdth_setlocsample_fishingmode_year_month_fleet_ocean_final$month_set))],
                      unique(avdth_setlocsample_fishingmode_year_month_fleet_ocean_final$year_set),
                      sep = "_")
  for (i in unique(avdth_setlocsample_fishingmode_year_month_fleet_ocean_final$month_set)) {
    tmp1 <- subset(avdth_setlocsample_fishingmode_year_month_fleet_ocean_final, month_set == i)
    tmp[[i]] <- ggplot2::ggplot() +
      ...
      ggplot2::ggtitle(label = paste0("Sampled sets location on ",
                                      fishing_mode,
                                      " for the ",
                                      fleet_name,
                                      " in the ",
