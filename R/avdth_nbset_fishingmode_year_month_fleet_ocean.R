#' @title Number of set by fishing mode, year, month, fleet and ocean
#' @description Number of set by fishing mode, year, month, fleet and ocean.
...
avdth_nbset_fishingmode_year_month_fleet_ocean <- function(avdth_con,
  ...
  # Data design ----
  if (unique(avdth_nbset_fishingmode_year_month_fleet_ocean$ocean) == 1) {
    ocean_name <- "Atlantic Ocean"
  } else {
    if (unique(avdth_nbset_fishingmode_year_month_fleet_ocean$ocean) == 2) {
      ocean_name <- "Indian Ocean"
    } else {
      if (unique(avdth_nbset_fishingmode_year_month_fleet_ocean$ocean) == 3) {
        ocean_name <- "West Pacific Ocean"
      } else {
        if (unique(avdth_nbset_fishingmode_year_month_fleet_ocean$ocean) == 4) {
          ocean_name <- "East Pacific Ocean"
        } else {
          if (unique(avdth_nbset_fishingmode_year_month_fleet_ocean$ocean) == 5) {
            ocean_name <- "Pacific Ocean"
          } else {
            if (unique(avdth_nbset_fishingmode_year_month_fleet_ocean_query$ocean) == 6) {
