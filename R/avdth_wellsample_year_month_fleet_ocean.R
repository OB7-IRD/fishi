...
avdth_wellsample_year_month_fleet_ocean <- function(avdth_con,
  ...
  if (unique(avdth_wellsample_year_month_fleet_ocean_final$ocean) == 1) {
    ocean_name <- "Atlantic Ocean"
  } else {
    if (unique(avdth_wellsample_year_month_fleet_ocean_final$ocean) == 2) {
      ocean_name <- "Indian Ocean"
    } else {
      if (unique(avdth_wellsample_year_month_fleet_ocean_final$ocean) == 3) {
        ocean_name <- "West Pacific Ocean"
      } else {
        if (unique(avdth_wellsample_year_month_fleet_ocean_final$ocean) == 4) {
          ocean_name <- "East Pacific Ocean"
        } else {
          if (unique(avdth_wellsample_year_month_fleet_ocean_final$ocean) == 5) {
            ocean_name <- "Pacific Ocean"
          } else {
            if (unique(avdth_wellsample_year_month_fleet_ocean_final$ocean) == 6) {
