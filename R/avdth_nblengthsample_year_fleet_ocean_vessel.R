#' @title Number of length sampled by year, fleet, ocean and vessel
#' @description Number of length sampled by year, fleet, ocean and vessel.
#' @name avdth_nblengthsample_year_fleet_ocean_vessel
...
#'                                              percentage = F)
#' @export
avdth_nblengthsample_year_fleet_ocean_vessel <- function(avdth_con,
                                                         year,
                                                         fleet,
                                                         ocean,
                                                         fleet_name,
  ...
  # Query importation ----
  avdth_nblengthsample_year_fleet_ocean_vessel_query <- paste(readLines(con = system.file("sql",
                                                                                          "avdth_nblengthsample_year_fleet_ocean_vessel.sql",
                                                                                          package = "fishi")),
                                                              collapse = "\n")

  # Value(s) interpolation(s) ----
  avdth_nblengthsample_year_fleet_ocean_vessel_query <- furdeb::sql_inset(db_type = "access",
                                                                   replacement = year,
                                                                   pattern = "year_interpolate",
                                                                   query = avdth_nblengthsample_year_fleet_ocean_vessel_query)
  ...
  # Data importation ----
  avdth_nblengthsample_year_fleet_ocean_vessel <- DBI::dbGetQuery(avdth_con,
                                                                  avdth_nblengthsample_year_fleet_ocean_vessel_query)

  # Data design ----
  if (unique(avdth_nblengthsample_year_fleet_ocean_vessel$ocean) == 1) {
    ocean_name <- "Atlantic Ocean"
  } else {
    if (unique(avdth_nblengthsample_year_fleet_ocean_vessel$ocean) == 2) {
      ocean_name <- "Indian Ocean"
    } else {
      if (unique(avdth_nblengthsample_year_fleet_ocean_vessel$ocean) == 3) {
        ocean_name <- "West Pacific Ocean"
      } else {
        if (unique(avdth_nblengthsample_year_fleet_ocean_vessel$ocean) == 4) {
          ocean_name <- "East Pacific Ocean"
        } else {
          if (unique(avdth_nblengthsample_year_fleet_ocean_vessel$ocean) == 5) {
            ocean_name <- "Pacific Ocean"
          } else {
            if (unique(avdth_nblengthsample_year_fleet_ocean_vessel$ocean) == 6) {
  ...
  avdth_nblengthsample_year_fleet_ocean_vessel$vessel_id = as.factor(avdth_nblengthsample_year_fleet_ocean_vessel$vessel_id)

  # Graphic design ----
  if (percentage %in% c("F", "FALSE", "False", "false")) {
    tmp <- ggplot2::ggplot(avdth_nblengthsample_year_fleet_ocean_vessel,
                           mapping = ggplot2::aes(fill = vessel_id,
                                                  y = nb_sample,
                                                  x = month_sample)) +
      ggplot2::geom_bar(stat = "identity") +
      ggplot2::scale_fill_brewer(palette = "Paired") +
      ggplot2::scale_x_discrete(limits = c(min(avdth_nblengthsample_year_fleet_ocean_vessel$month_sample):max(avdth_nblengthsample_year_fleet_ocean_vessel$month_sample))) +
